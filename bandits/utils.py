...
def betaincinv(a, b, p):
    ...
    def loop_fn(t, carry):
        x, a, b, p, a1, b1, afac, stop = carry
        x_new, t = update_x(x, a, b, p, a1, b1, afac)
        x_res = jnp.where(stop, x, x_new)
        stop_new = jnp.where(jnp.logical_or(jnp.abs(t) < ERROR * x_res, stop), True, False)

        return (x_res, a, b, p, a1, b1, afac, stop_new)

    res = lax.fori_loop(0, 10, loop_fn, (x, a, b, p, a1, b1, afac, stop))
